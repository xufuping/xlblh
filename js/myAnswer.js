...
//分页按钮
	...
	(function(){
		...
		$.ajax({
			type : 'GET',
			url : '/getQuestionCommentNumber.do',
            data : {
              userId : $.cookie(COOKIE_NAME_userId)
            },
			success : function(data){
            ...
            success : function(data){
                ...
                if (data.status === '0') {
                    ...
                    for(var i=0;i<pageData.length;i++){
                        answerContentStr +='<p class="mypost_content_li"><span class="Post_Name" id="Post_Name"><a href = "discussion.html?articleId="'+ pageData[i].articleid +'>'+ pageData[i].title
                            +'</a></span><span class="Post_Username_1" id="Username">'+ pageData[i].userName
                            // +'</span><span class="Post_Username_2" id="Username">'+ pageData[i].lastUser
                            +'</span><span class="Post_Date" id="Date">'+ pageData[i].createdate
                            +'</span></p>';
                    }
                $('.mypost_content').html(answerContentStr);
                }    
            }
        });
